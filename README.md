

# Sui Move Smart Contract Engineer

Focused on **Move smart contracts, DeFi, and smart contract security**.

## Audit Contests

|  № | Project          |   TVL | Findings | Language | Profile                                                                                                                                    |
| -: | ---------------- | ----: | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
|  2 | Bluefin Protocol | $19M+ | 1 Unique | Move     | [Finding](https://github.com/gurwindersinghdev/vul.git)                                                                                    |
|  1 | Aave Protocol    | $10B+ | 2 Unique | Move     | [Finding 1](https://github.com/gurwindersinghdev/unique-vul.git) / [Finding 2](https://github.com/gurwindersinghdev/unique-vul-part-2.git) |

## Private Security Work

|  № |   TVL | Vulnerabilities | Report | Language   |
| -: | ----: | --------------- | ------ | ---------- |
|  2 | $5.2M | 2 High          | NDA    | Sui (Move) |
|  1 |  $50M | 1 Critical      | NDA    | Sui (Move) |



## Other Work


* **Security Contributions:** [Security Contributions: Confidential / NDA](https://github.com/gurwindersui)
* **Full-Stack Solidity Projects:** [DeFi Projects](https://defiprojects.vercel.app/)
* **YouTube:** [GS Developer](https://www.youtube.com/@Gsdeveloper33/videos)
* **Sui Move Security Articles:** [11+ articles on X](https://x.com/gurwinder_sui/articles)




